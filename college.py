...
API_KEY="AIzaSyByLfEfaGdpdZHHdcPgbQeRAISAFqFrv4s"
...
st.write("Welcome to College Chatbot!")

for message in st.session_state.messages:
  with st.chat_message(message["role"]):
    st.markdown(message["content"])
    
if prompt := st.chat_input("Say your thoughts..."):
  ...
  if closest_answer:
    ...
    try:
      response =model.generate_content(
        f"You are helpful and knowledgeable chatbot for SVECW College. Provide a detailed  answer to the following question:{prompt}")
